...
function TagPontos({ pontos }: { pontos: number }) {
  const cls =
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="bg-[#002776]/5 px-4 py-2.5 flex items-center justify-between gap-2">
        ...
        <TagPontos pontos={palpite.pontos} />
      </div>
...
export default async function PerfilPublicoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Se for o próprio perfil, redireciona para /perfil
  ...
  const palpites = ((palpitesData ?? []) as PalpiteComJogo[]).filter(p => p.jogo?.status === 'encerrado')

  const acertos  = palpites.filter(p => p.pontos === 3).length
  const parciais = palpites.filter(p => p.pontos === 1).length
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      {/* Header temático */}
      <header
        className="px-4 pt-10 pb-8 shadow-lg relative"
        style={{ background: `linear-gradient(160deg, ${corFundo} 0%, ${corFundo}cc 100%)` }}
      >
        {/* Botão voltar */}
        <Link
          href="/ranking"
          className="absolute top-12 left-4 w-9 h-9 rounded-full bg-white/20 flex items-center justify-center active:bg-white/30 transition-colors"
        >
